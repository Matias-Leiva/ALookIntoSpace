...
const Pagination = ({rovers})=> {
    ...
    const handleChangePage = (direction) => {
        let page = rovers.querys.page
        dispatch(setPage(direction === 'left' ? --page : ++page));
    }

    useEffect(() => {
        (rovers.images.length < 25) ? setDisabledRight(true) : setDisabledRight(false);
    ...
    return (
        <Grid container justifyContent='center' alignItems='center'>
            <Grid item sm={1} className='justify_center'>
                <IconButton size="small" onClick={() => handleChangePage('left')} disabled={disabledLeft} >
            ...
            <Grid item sm={1} className='justify_center'>
                <IconButton size="small" onClick={() => handleChangePage('right')} disabled={disabledRight}>
